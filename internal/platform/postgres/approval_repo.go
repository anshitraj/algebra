...
func (r *ApprovalRepo) Create(ctx context.Context, a *approval.Approval) error {
	_, err := r.db.Pool.Exec(ctx, `
		INSERT INTO approvals (id, intent_id, quote_id, user_id, agent_id, merchant, amount_minor_units, currency,
		...
		a.ID, a.IntentID, a.QuoteID, a.UserID, a.AgentID, a.Merchant, a.Amount.MinorUnits, a.Amount.Currency,
...
const approvalSelectSQL = `
	SELECT id, intent_id, quote_id, user_id, agent_id, merchant, amount_minor_units, currency,
...
func scanApproval(row pgx.Row) (*approval.Approval, error) {
	...
	var status string
	err := row.Scan(&a.ID, &a.IntentID, &a.QuoteID, &a.UserID, &a.AgentID, &a.Merchant,
