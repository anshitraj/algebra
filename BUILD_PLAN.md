...
|---|---|---|
| OmniClaw (`omniclaw.ai`, `github.com/omnuron/omniclaw`) | Real project, **vendored as a git submodule at `third_party/omniclaw`** (v0.0.8) so the integration below is built against actual source, not marketing copy. Confirmed by reading it directly: a Python financial policy engine + non-custodial payment router for agent buyers, covering two rails — `circle_transfer` (Circle Developer Wallet) and `x402` (paid HTTP endpoints / Gateway nanopayments). Runs as a FastAPI service (`uvicorn omniclaw.agent.server:app`, port 9091 in `third_party/omniclaw/examples/agent/buyer/docker-compose.yml`). Its policy schema (`src/omniclaw/agent/policy_schema.py`) is keyed on a wallet-address-or-URL `recipient`, a USDC decimal `amount`, and a `confirm_threshold` — there is genuinely no merchant name, INR amount, or product-category field anywhere in it. | `internal/domain/policy/omniclaw_provider.go` is a **real, tested HTTP client** against the verified endpoints (`GET /health`, `GET /wallets`, `GET /can-pay`, `POST /simulate`, `POST /pay`, `POST /confirmations/{id}/approve\|deny`, all under `/api/v1`, `Authorization: Bearer <OMNICLAW_TOKEN>`). `EvaluatePurchaseIntent`/`EvaluatePayment`/`EvaluateMerchant` make real calls when `policy.Input.CryptoRecipient`/`AmountUSDC` are set (i.e. the resolved `PaymentSource` is a non-custodial crypto rail); `EvaluateAmount`/`EvaluateCategory`/`EvaluatePaymentSource` return a descriptive error rather than a fabricated answer, because OmniClaw genuinely has no single-field equivalent for those. `confirm_threshold` is read from `GET /wallets` and reproduces OmniClaw's own `requires_confirmation()` gate client-side (see the execute-time-never-re-requires-approval regression test — the same bug class fixed in `LocalProvider`). Verified against a fake server built from the vendored source's exact response shapes (`omniclaw_provider_test.go`); **not yet verified against a real running instance** — that needs `OMNICLAW_AGENT_TOKEN`/`CIRCLE_API_KEY`/`OMNICLAW_PRIVATE_KEY`/etc. (see `.env.example`), none of which exist in this environment. `LocalPolicyProvider` remains the default and the only provider that understands merchants/categories/INR at all — the two are complementary, not competing: OmniClaw for a crypto `PaymentSource`, `LocalProvider` for everything else. `Pay`/`ApproveConfirmation`/`DenyConfirmation` are implemented and tested but not yet called from `OrderService` — wiring an actual crypto-rail execution branch into `Execute` is the natural next step, not done in this session. |
...
| Amazon Creators API | The official Associates catalog API that replaced PA-API 5.0 (retired May 2026). Search/get items only; Amazon offers no third-party cart or order API. | `connectors/amazon`: real `searchItems` client (client-credentials token per credential version; India uses 3.2 or 2.2). Tested against a fake server; not run against live credentials (none here). |
...
- **REAL**: `PurchaseIntent` state machine, Postgres schema/migrations, append-only audit log, `AgentIdentity` + scoped permissions + revocation, `LocalPolicyProvider` rule engine, `PrivacyResolver` + AES-256-GCM envelope encryption (`LocalEncryptedProvider`), approval binding/hashing, idempotency-key middleware, MCP server (official Go SDK, real tool surface), REST API v1, quote/effective-cost calculator, structured logging with redaction.
